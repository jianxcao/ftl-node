		...
		<dl class="nav" data-group-name="<%=data[i].groupName%>">
			<%if (data[i].branches && data[i].branches.length) {%>
				<%for (var j = 0; j < data[i].branches.length; j++) {%>
					<dd data-fun="project" data-branch-name="<%=data[i].branches[j].branchName%>" <%if (data[i].branches[j].disabled){%>class="disabled"<%}%> ><%=data[i].branches[j].branchName%><span class="glyphicon glyphicon-ban-circle dis-icon"></span><span class="glyphicon glyphicon-trash del-icon"></span></dd>
				<%}%>
			<%}%>
		</dl>
	</dd>
